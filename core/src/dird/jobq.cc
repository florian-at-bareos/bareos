...
int JobqAdd(jobq_t* jq, JobControlRecord* jcr)
{
  ...
  Dmsg3(2300, "JobqAdd jobid={} jcr={:p} UseCount={}\n", jcr->JobId, jcr,
        jcr->UseCount());
...
int JobqRemove(jobq_t* jq, JobControlRecord* jcr)
{
  ...
  Dmsg2(2300, "JobqRemove jobid={} jcr={:p}\n", jcr->JobId, jcr);
  if (jq->valid != JOBQ_VALID) { return EINVAL; }
  ...
  if (!found) {
    unlock_mutex(jq->mutex);
    Dmsg2(2300, "JobqRemove jobid={} jcr={:p} not in wait queue\n", jcr->JobId,
          jcr);
    return EINVAL;
  }
  ...
  jq->ready_jobs->prepend(item);
  Dmsg2(2300, "JobqRemove jobid={} jcr={:p} moved to ready queue\n", jcr->JobId,
        jcr);
