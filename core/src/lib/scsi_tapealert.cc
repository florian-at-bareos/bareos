...
static tapealert_mapping tapealert_mappings[] = {
    ...
    {0x0D,
     (const char*)"Recoverable mechanical cartridge failure (broken tape?)"},
    {0x0E,
     (const char*)"Unrecoverable mechanical cartridge failure (broken tape?)"},
    ...
    {0x1E, (const char*)"Hardware A: Tape drive has a problem not read/write "
                        "related"},
    {0x1F, (const char*)"Hardware B: Tape drive has a problem not read/write "
                        "related"},
    {0x20, (const char*)"Problem with SCSI interface between tape drive and "
                        "initiator"},
    {0x21,
     (const char*)"The current operation has failed. Eject and reload media"},
    ...
    {0x28, (const char*)"Loader Hardware A: Changer having problems "
                        "communicating with tape drive"},
    {0x29, (const char*)"Loader Stray Tape: Stray tape left in drive from "
                        "prior error"},
    ...
    {0x33,
     (const char*)"Tape directory invalid at unload (reread all to rebuild)"},
...
bool GetTapealertFlags(int fd, const char* device_name, uint64_t* flags)
{
  ...
  *flags = 0;
  ...
  if ((cmd_page.pagecode & 0x3f) != SCSI_TAPE_ALERT_FLAGS) { return false; }

  tapealert_length = (cmd_page.page_length[0] << 8) + cmd_page.page_length[1];
  if (!tapealert_length) { return true; }
